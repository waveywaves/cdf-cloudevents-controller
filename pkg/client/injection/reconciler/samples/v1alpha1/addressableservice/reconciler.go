...
// controller reconciling v1alpha1.AddressableService if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1alpha1.AddressableService.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1alpha1.AddressableService) reconciler.Event
}

// ReadOnlyFinalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.AddressableService if they want to process tombstoned resources
...
type ReadOnlyFinalizer interface {
	// ObserveFinalizeKind implements custom logic to observe the final state of v1alpha1.AddressableService.
	// This method should not write to the API.
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister samplesv1alpha1.AddressableServiceLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, existing *v1alpha1.AddressableService, desired *v1alpha1.AddressableService) error {
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.AddressableService) (*v1alpha1.AddressableService, error) {

	getter := r.Lister.AddressableServices(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1alpha1.AddressableService) (*v1alpha1.AddressableService, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1alpha1.AddressableService, reconcileEvent reconciler.Event) (*v1alpha1.AddressableService, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
