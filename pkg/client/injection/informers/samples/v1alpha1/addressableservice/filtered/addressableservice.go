...
func Get(ctx context.Context, selector string) v1alpha1.AddressableServiceInformer {
	untyped := ctx.Value(Key{Selector: selector})
	if untyped == nil {
		logging.FromContext(ctx).Panicf(
			"Unable to fetch github.com/waveywaves/cloudevents-controller/pkg/client/informers/externalversions/samples/v1alpha1.AddressableServiceInformer with selector %s from context.", selector)
	}
